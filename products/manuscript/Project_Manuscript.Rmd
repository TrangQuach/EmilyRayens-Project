...
This is a summary of the values of the Pc Diagnostic Variables by Identifier. I am struggling to display the table I generated in the analysisscript and what I can see within the RMarkdown document when I run the chunk of code. Any assistance on this is appreciated.  
```{r,  echo=FALSE}
...
```{r,  echo=FALSE}
resulttable2 <- readRDS("~/Documents/GitHub/epid8060fall2019/EmilyRayens-Project/results/resulttable2.rds")
resulttable2.m<-as.matrix(resulttable2)
resulttable2.df<-as.data.frame(resulttable2.m)
knitr::kable(resulttable2.df, caption = 'Summary of Charateristics by Vaccine Status')
```
...
When starting to address our questions, we want to plot the data to get a sense of what is there. First and foremost, we want to adress is experimental vaccination reduces the incidence of Pc in this cohort.
...
```
We can see here that other than a signle point (which may prove to be an outlier on further examination), our vaccinated cohort has lower incidence of Pc when plotted. We can see smaller numbers at the 1 and 2 (more severe) categorizations in this cohort. We will evaluate in the full analysis if this is a significant difference. 
...
```
Does experimental vaccination reduce the incidence of Pc in this cohort?
